...
use crate::domain::models::{
    ExtensionFilterMode, SearchContentMode, SearchFinished, SearchProgress, SearchQuery,
    SearchResult,
};
    ...
    for _ in 0..worker_count {
        ...
        handles.push(thread::spawn(move || loop {
            ...
            let job = match job_receiver
                .lock()
                .expect("content search queue poisoned")
                .recv()
            {
                Ok(job) => job,
                Err(_) => break,
...
fn is_allowed_extension(path: &Path, extensions: &[String], mode: &ExtensionFilterMode) -> bool {
    ...
    let extension = path
        ...
        .unwrap_or_default()
        .to_lowercase();
    let matched = extensions.iter().any(|candidate| {
        candidate
            .trim_start_matches('.')
        ...
        for entry in walker.into_iter().filter_map(|entry| entry.ok()) {
            ...
            if let Some(workers) = &content_workers {
                if scanned_entries % RESULT_DRAIN_INTERVAL == 0 {
                    drain_content_results(
                        &workers.result_receiver,
                        &mut matched_entries,
                        &mut on_result,
                    )?;
                }
...
mod tests {
    ...
    fn name_search_modes_extension_filters_and_folder_inclusion_are_applied() {
        ...
        assert!(excluded_results
            .iter()
            .any(|result| result.name == "release-notes.txt" && !result.is_directory));
        assert!(excluded_results
            .iter()
            .any(|result| result.name == "release-candidate" && result.is_directory));
        assert!(!excluded_results
            .iter()
            .any(|result| result.name == "release-manifest.json"));
    ...
    fn search_respects_recursive_flag() {
        ...
        assert!(non_recursive_results
            .iter()
            .any(|result| result.name == "root.txt"));
        assert!(!non_recursive_results
            .iter()
            .any(|result| result.name == "nested.txt"));
    ...
    fn content_search_supports_regex_mode() {
        ...
        fs::write(root.join("skip.txt"), "request error completed")
            .expect("write non matching search file");
