...
use crate::{
    domain::models::{
        ConflictResolutionKind, OperationConflictRequest, OperationConflictResolution,
        OperationEntryKindSnapshot, OperationEntryResult, OperationEntryResultKind, OperationError,
        OperationErrorCode, OperationErrorSource, OperationHistoryEventEnvelope,
        OperationHistoryListSnapshot, OperationHistoryRecord, OperationHistoryStatus,
        OperationIntent, OperationIntentKind, OperationPathRef, OperationTaskEventEnvelope,
        OperationTaskListSnapshot, OperationTaskSnapshot, OperationTaskStatus,
    },
    services::fs_service,
};
...
impl OperationStore {
    ...
    pub fn queue_operation(&mut self, intent: OperationIntent) -> (OperationServiceResult, bool) {
        ...
        self.request_to_task
            .insert(intent.request_id.clone(), task_id.clone());
    ...
    pub(crate) fn finish_operation(
        ...
        task.cancelable = matches!(
            task.status,
            OperationTaskStatus::WaitingConflict | OperationTaskStatus::Running
        );
        ...
        if let Some(conflict) = &execution.conflict {
            ...
            let event =
                self.create_history_record(&task, payload, OperationHistoryStatus::Undoable, None);
        ...
        if !matches!(
            task.status,
            OperationTaskStatus::WaitingConflict
    ...
    pub fn prepare_undo_record(
        ...
        if !matches!(
            self.history[record_index].status,
    ...
    pub fn finish_undo_operation(
        ...
        task.completed_entries = task
            .entry_results
            .len()
            .saturating_sub(result.failed_entries);
...
fn execute_local_intent(
    ...
    match intent.kind {
        ...
        OperationIntentKind::Move => execute_copy_or_move(
            task_id,
            intent,
            app_data_dir,
            cancellation,
        ...
        OperationIntentKind::CreateFile => execute_create(
            task_id,
            intent,
            app_data_dir,
            cancellation,
...
fn execute_copy_or_move_entry(
    ...
    if decision.merge_directory {
        let mut undo_actions =
            merge_directory_exact(source, &decision.destination, move_source, cancellation)?;
...
fn create_conflict_request(
    task_id: &str,
    source: OperationPathRef,
    destination: &Path,
...
fn intent_has_remote_path(intent: &OperationIntent) -> bool {
    intent
        ...
        .map(|sources| {
            sources
                .iter()
                .any(|source| matches!(source, OperationPathRef::Remote { .. }))
        })
        .unwrap_or(false)
...
mod tests {
    ...
    use crate::domain::models::{
        ConflictResolutionKind, OperationConflictResolution, OperationHistoryStatus,
    ...
    fn operation_journal_load_tolerates_corrupt_file() {
        ...
        let store = OperationStore::load_from(journal_path.clone())
            .expect("corrupt journal should not block startup");
    ...
    fn delete_operation_uses_app_managed_trash_and_undo_restores_original() {
        ...
        assert_eq!(undo.snapshot.status, OperationTaskStatus::Succeeded);
        assert_eq!(
            fs::read_to_string(deleted_path).expect("read restored"),
            "hello"
        );
    ...
    fn copy_conflict_waits_for_resolution_and_keep_both_uses_numbered_destination() {
        ...
        assert_eq!(
            fs::read_to_string(destination.join("report.txt")).expect("read existing"),
            "existing"
        );
        assert_eq!(
            fs::read_to_string(destination.join("report (1).txt")).expect("read copy"),
            "incoming"
        );
    ...
    fn directory_conflict_allows_merge_directory_and_merges_children() {
        ...
        fs::write(destination.join("docs").join("existing.txt"), "existing")
            .expect("write existing");

        let mut store = OperationStore::default();
        let result = store.start_operation(
            copy_intent(&source.join("docs"), &destination),
            Some(root.clone()),
        );
        assert_eq!(result.snapshot.status, OperationTaskStatus::WaitingConflict);
        ...
        assert_eq!(resolved.snapshot.status, OperationTaskStatus::Succeeded);
        assert_eq!(
            fs::read_to_string(destination.join("docs").join("existing.txt"))
                .expect("read existing"),
            "existing"
        );
        assert_eq!(
            fs::read_to_string(destination.join("docs").join("incoming.txt"))
                .expect("read incoming"),
            "incoming"
        );

        let undo = store
            .undo_latest("request-undo-merge".into())
            .expect("undo merge copy");
        assert_eq!(undo.snapshot.status, OperationTaskStatus::Succeeded);
    ...
    fn copy_conflict_without_apply_to_all_prompts_again_for_next_conflict() {
        ...
        let result = store.start_operation(
            OperationIntent {
                ...
                kind: OperationIntentKind::Copy,
                sources: Some(vec![
                    local(&source.join("a.txt")),
                    local(&source.join("b.txt")),
                ]),
                destination: Some(local(&destination)),
                source_path: None,
                new_name: None,
        ...
        assert_eq!(
            second_wait.snapshot.status,
            OperationTaskStatus::WaitingConflict
        );
        assert_eq!(
            fs::read_to_string(destination.join("a (1).txt")).expect("read copied a"),
            "incoming-a"
        );
        ...
        assert_eq!(resolved.snapshot.status, OperationTaskStatus::Succeeded);
        assert_eq!(
            fs::read_to_string(destination.join("b (1).txt")).expect("read copied b"),
            "incoming-b"
        );
