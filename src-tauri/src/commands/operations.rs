...
pub fn copy_entries(request: FileOperationRequest) -> Result<OperationResult, String> {
    ...
    for source in request.sources {
        ...
        let copied = fs_service::copy_recursively(source_path, &destination)
...
pub fn move_entries(request: FileOperationRequest) -> Result<OperationResult, String> {
    ...
    for source in request.sources {
        ...
        let destination = Path::new(&destination_root).join(file_name);
        let moved =
            fs_service::move_entry(source_path, &destination).map_err(|error| error.to_string())?;
...
#[tauri::command]
pub fn list_file_operation_tasks(
    state: State<'_, Arc<AppState>>,
) -> Result<OperationTaskListSnapshot, String> {
...
pub fn list_operation_history(
    state: State<'_, Arc<AppState>>,
) -> Result<OperationHistoryListSnapshot, String> {
    let operations = state
        .operations
        .lock()
        .expect("operation store lock poisoned");
