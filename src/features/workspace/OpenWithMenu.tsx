...
import { findMenuTrigger, focusMenuItem, menuAnchor, parentMenuContains, positionMenu, useMenuBranchHover } from "./menuInteraction";
...
export interface OpenWithMenuProps {
  ...
  onClose: (requestId: string, keepParent?: boolean) => void;
}
...
export function OpenWithMenu({ menu, rules, onSelect, onConfirm, onClose }: OpenWithMenuProps) {
  ...
  const collapse = () => {
    if (root.current?.contains(document.activeElement)) focusMenuItem(findMenuTrigger(menu.parent));
    onClose(menu.requestId, true);
  };
  useMenuBranchHover(menu.parent, root, collapse);
  const measure = useCallback(() => {
    ...
    const rowAnchor = (row ?? listing)?.getBoundingClientRect();
    const trigger = findMenuTrigger(menu.parent);
    const anchor = trigger ? menuAnchor(trigger) : menu.anchor ?? { x: rowAnchor?.left ?? 8, y: rowAnchor?.top ?? 8 };
    const rect = surface.getBoundingClientRect();
    const { left, top } = positionMenu(anchor, rect);
    setPosition(previous => previous.left === left && previous.top === top ? previous : { left, top });
  }, [menu.panelId, menu.path, menu.parent, menu.anchor]);
  ...
  useEffect(() => {
    const outside = (event: PointerEvent) => {
      if (event.target instanceof Node && !root.current?.contains(event.target) && !parentMenuContains(event.target, menu.parent)) onClose(menu.requestId);
    };
  ...
  return createPortal(
    <MenuSurface ref={root} className="open-with-menu" data-menu-owner={menu.parent?.hostId} role="menu" aria-label="打开方式" tabIndex={-1}
      ...
      onKeyDown={event => {
        ...
        else if (event.key === "ArrowLeft" && menu.parent) collapse();
      ...
      <div className="open-with-menu__choices" ref={choices}>
        ...
        {menu.ruleIds.map((id, index) => {
          ...
          return <button key={id} id={`${menu.requestId}-${index}`} type="button" role="menuitem" tabIndex={-1}
            className={`app-menu__item open-with-menu__item${menu.selectedIndex === index ? " is-selected" : ""}`}
      ...
      <div className="open-with-menu__footer">
        <button id={`${menu.requestId}-${menu.ruleIds.length}`} type="button" role="menuitem" tabIndex={-1}
          className={`app-menu__item open-with-menu__item${menu.selectedIndex === menu.ruleIds.length ? " is-selected" : ""}`}
