...
export const completion = (async () => {
  ...
  Reflect.set(state.settings.model, "treeAutoFollowEnabled", false);
  ...
  const checkbox = () => {
    const element = container.querySelector<HTMLInputElement>('[data-setting-id="tree-auto-follow-enabled"]');
    assert.ok(element, "文件列表设置应提供目录树自动跟踪开关");
    return element;
  };
  ...
  try {
    await assertTest("tree auto follow setting stays in the draft until Confirm and marks File List dirty", async () => {
    ...
    await assertTest("Cancel discards tree auto follow draft and opening settings preserves persisted true", async () => {
      saved.length = 0;
      Reflect.set(state.settings.model, "treeAutoFollowEnabled", true);
      await render("cancel");
      assert.equal(checkbox().checked, true);
      await click(checkbox());
      assert.equal(checkbox().checked, false);
      ...
      assert.equal(saved.length, 0);
      assert.equal(Reflect.get(state.settings.model, "treeAutoFollowEnabled"), true);
    });
  } finally {
