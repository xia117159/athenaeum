...
for (const viewMode of ["details", "large-icons", "tiles", "list"] as TabViewMode[]) {
  ...
  assert.equal(getFolderListingRows(tab, undefined, "other-name").length, 0);
  ...
  assert.equal(getFolderListingRows(tab, undefined, "other-name")[0]?.entry.id, entry.id, `${viewMode}: reveal active inline editor through a quick filter`);
  tab.inlineEdit = undefined;
  assert.equal(getFolderListingRows(tab, undefined, "other-name").length, 0, "cancel restores original filtering");
}
